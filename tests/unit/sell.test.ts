...
import { buyersInCity, isBuyer, sellCityCapsule, sellCityFaqs, sellPageExistsForCity } from '../../src/lib/sell';
 ...
 *
 * Uses the file's existing `store()` fixture (with explicit citySlug
 * overrides) rather than a separate fixture, to keep one store-builder per
 * file. Original plan-11 test IDs are unchanged; only the fixture wiring is
 * adapted to coexist with the pre-existing tests above, which this file
 * already had before plan 11 (the plan's baseline that this was a brand-new
 * file was stale).
 */
describe('sellPageExistsForCity', () => {
  it('is true when a store in that city buys collections', () => {
    expect(sellPageExistsForCity([store({ citySlug: 'edmonton', services: ['Buys'] })], 'edmonton')).toBe(true);
  });

  it('is false when no store in that city buys collections', () => {
    expect(sellPageExistsForCity([store({ citySlug: 'edmonton', services: ['Sells'] })], 'edmonton')).toBe(false);
  });

  it('is false for a city with no stores at all', () => {
    expect(sellPageExistsForCity([store({ citySlug: 'edmonton', services: ['Buys'] })], 'calgary')).toBe(false);
  });

  it('matches case- and whitespace-insensitively, same as isBuyer', () => {
    expect(
      sellPageExistsForCity([store({ citySlug: 'edmonton', services: [' buys ' as never] })], 'edmonton'),
    ).toBe(true);
  });

  it('is true if ANY store in the city buys, even if others in the same city do not', () => {
    const stores = [
      store({ slug: 'a', citySlug: 'edmonton', services: ['Sells'] }),
      store({ slug: 'b', citySlug: 'edmonton', services: ['Buys'] }),
    ];
